package constants;

public final class ConstRequestProperty {
	public static final String CONTENT_TYPE = "Content-Type";
	public static final String CONTENT_TYPE_STANDART_VALUE = "application/x-www-form-urlencoded";
	public static final String AUTHORIZATION = "Authorization";
	public static final String POST_METHOD = "POST";
	public static final String GET_METHOD = "GET";	
	public static final String HP_CLIENT_TYPE = "HPECLIENTTYPE";
	public static final String HP_CLIENT_TYPE_VALUE = "HPE_MQM_UI";		
	public static final String CONTENT_TYPE_JSON_VALUE = "application/json";
	public static final String HP_HEADER_CSRF = "HPSSO_HEADER_CSRF";	/// oop! hello!
	 
	public static final String URL_AUTH = "authentication/sign_in";
	public static final String URL_SHARED_SPACE = "api/shared_spaces/"; // hi
	public static final String URL_WORKSPACE = "/workspaces/";
}
