...
public class EntityModelEditor extends EditorPart {
    ...
    public void init(IEditorSite site, IEditorInput input) throws PartInitException {
        ...
        try {
            entityModel = TemporaryDataProvider.findEntity(this.input.getEntityType(), this.input.getId());
        } catch (ServiceException e1) {
            e1.printStackTrace();
        }

        entityType = Entity.getEntityType(entityModel);
        entityId = Long.parseLong(entityModel.getValue("id").getValue().toString());

        try {
            entityModel = TemporaryDataProvider.findEntity(entityType, entityId);
